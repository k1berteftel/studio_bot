...
async def form_choose(clb: CallbackQuery, widget: Button, dialog_manager: DialogManager):
    dialog_manager.dialog_data['form_type'] = clb.data.split('_')[0]
    await dialog_manager.switch_to(adminSG.get_form_id)


async def get_form_id(msg: Message, widget: ManagedTextInput, dialog_manager: DialogManager, text: str):
    try:
        form_id = int(text)
    except Exception:
        await msg.answer('ID формы должен быть числом, пожалуйста попробуйте еще раз')
        return
    session: DataInteraction = dialog_manager.middleware_data.get('session')
    form_type = dialog_manager.dialog_data.get('form_type')
    if form_type == 'diagnostic':
        form = await session.get_diagnostic_form_by_id(form_id)
    else:
        form = await session.get_consult_form_by_id(form_id)
    if not form:
        await msg.answer('К сожалению формы с таким ID н найдено')
        return
    dialog_manager.dialog_data['form_id'] = form_id
    await dialog_manager.switch_to(adminSG.show_form_menu)


async def show_form_getter(dialog_manager: DialogManager, **kwargs):
    form_id = dialog_manager.dialog_data.get('form_id')
    session: DataInteraction = dialog_manager.middleware_data.get('session')
    form_type = dialog_manager.dialog_data.get('form_type')
    text = '<b>Форма на бизнес-диагностику</b>' if form_type == 'diagnostic' else '<b>Форма на консультацию</b>'
    if form_type == 'diagnostic':
        form = await session.get_diagnostic_form_by_id(form_id)
        user = await session.get_user(form.user_id)
        text += (f'\nЮзернейм: {"@" + user.username if user.username else "-"}\nНиша: {form.niche}\nКлючевая '
                 f'боль: {form.pain}\nМасштаб проблемы (количественная оценка): {form.problem}\nУровень '
                 f'цифровизации: {form.digitalization}\nВлияние на финансы: {form.finance}\nЦель для автоматизации: '
                 f'{form.purpose}')
    else:
        form = await session.get_consult_form_by_id(form_id)
        user = await session.get_user(form.user_id)
        text += (f'\nЮзернейм: {"@" + user.username if user.username else "-"}\nФокус консультации: {form.focus}\n'
                 f'Уровень текущего процесса: {form.process}\nОжидания по срокам и бюджетам: {form.deadline}\n'
                 f'Технические возможности: {form.features}\nКритерии успеха консультации: {form.criteria}')
    return {
        'text': text
    }
