...
admin_dialog = Dialog(
    Window(
        ...
        Column(
            ...
            SwitchTo(Const('👀Просмотр данных форм'), id='forms_menu_switcher', state=adminSG.form_menu),
    ...
    Window(
        Const('Выберите вид формы, которую вы хотели бы просмотреть'),
        Column(
            Button(Const("Диагностика"), id='diagnostic_form_choose', on_click=getters.form_choose),
            Button(Const("Консультация"), id='consult_form_choose', on_click=getters.form_choose),
        ),
        SwitchTo(Const('🔙 Назад'), id='back', state=adminSG.start),
        state=adminSG.form_menu
    ),
    Window(
        Const('Введите номер формы'),
        TextInput(
            id='get_form_id',
            on_success=getters.get_form_id
        ),
        SwitchTo(Const('🔙 Назад'), id='back_form_menu', state=adminSG.form_menu),
        state=adminSG.get_form_id
    ),
    Window(
        Format('{text}'),
        SwitchTo(Const('🔙 Назад'), id='back_form_menu', state=adminSG.form_menu),
        getter=getters.show_form_getter,
        state=adminSG.show_form_menu
    ),
    Window(
